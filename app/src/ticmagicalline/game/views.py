    ...
    if user is not None and user.is_active:
        ...
        if User.objects.filter(username=request.POST.get("username", "")).exists():
            ...
            if (
                ...
                or request.POST.get("password", "").find(
                    request.POST.get("username", "")
                )
                != -1
            ):
                return makeErrorView(
                    request, "Password too short or too similar to username."
                )
    ...
    if game is not None:
        ...
        if lastPlayer == None and game.user_o == user:
            ...
            if game.winner == "D":
                ...
                context["currentStatus"] = "The game has ended! You wins!"
            else:
                context["currentStatus"] = "The game has ended! Your enemy wins!"
